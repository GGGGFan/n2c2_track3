    ...
    def convert_to_list(self):
        ...
        for i, df_data in enumerate(data_iter):
            ...
            for rid, row in df_data.iterrows():
                ...
                encoded_dict['prev_pred'] = [0, 0, 0, 0]
...
class Transformer_base(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.linear_1 = nn.Linear(768+1+3+4+4, 4)
        self.act_1 = nn.ReLU()
        self.drop_1 = nn.Dropout(args.dropout)
        self.softmax = nn.LogSoftmax(dim=1)

    def forward(self, x, mask_attention, token_type_ids, added_fts, cum_pred, prev_pred):
        ...
        # disposition
        out = self.linear_1(torch.cat([out, added_fts, cum_pred, prev_pred], dim=1))
        out = self.drop_1(out)
...
def data_iter(data, batch_size, is_shuffle=True):
    ...
    for i in range(batch_num):
        ...
        prev_pred = [data[i * batch_size + b]['prev_pred'] for b in range(cur_batch_size)]
        yield sents, labels, mask_attention, token_type_ids, added_fts, cum_pred, prev_pred

def model_eval(lst_data, model, dataiter, mode='train'):
    """
    Given a list of dictionary and a model, make predictions and return macro-F1
    """
    list_yTrue = []
    list_yPred = []
    list_probPred = []
    model.eval()
    if mode == 'train':
        for sents, labels, mask_attention, token_type_ids, added_fts, cum_pred, prev_pred in data_iter(lst_data, batch_size=4, is_shuffle=False):
            ...
            prev_pred = torch.FloatTensor(prev_pred).to(device)
            # predict
            y_pred_ary = torch.exp(model(X, mask_attention, token_type_ids, added_fts, cum_pred, prev_pred)).detach().cpu().numpy()
            y_pred = np.argmax(y_pred_ary, axis=1)
            # append to list
            list_yTrue.extend(labels)
            list_yPred.extend(y_pred)
            list_probPred.extend(y_pred_ary)
        return f1_score(list_yTrue, list_yPred, average='micro'), list_yTrue, list_yPred, list_probPred
    else:
        for sents, labels, mask_attention, token_type_ids, added_fts, _, _ in data_iter(lst_data, batch_size=1, is_shuffle=False):
            X = torch.LongTensor(sents).to(device)
            token_type_ids = torch.LongTensor(token_type_ids).to(device)
            mask_attention = torch.LongTensor(mask_attention).to(device)
            added_fts = torch.FloatTensor(added_fts).to(device)
...
if __name__ == "__main__":
    ...
    print("Device:", device)
    with open('tuning_model4.txt', 'a') as f:
    ...
    for rid, sample in enumerate(dataset.train_lst):
        if rid+1 != len(dataset.train_lst) and rid+1 not in dataset.idx_start_train:
            ...
            dataset.train_lst[rid+1]['prev_pred'][sample['label']] = 1
        if rid in dataset.idx_start_train:
            dataset.train_lst[rid]['prev_pred'][3] = 1
    # initialize model
    ...
    for ep in range(args.epoch):
        ...
        train_loss = 0
        num_batches = 0
        for sents, labels, mask_attention, token_type_ids, added_fts, cum_pred, prev_pred in data_iter(dataset.train_lst, batch_size=args.batch_size):
            # convert to tensors
            X = torch.LongTensor(sents).to(device)
            mask_attention = torch.LongTensor(mask_attention).to(device)
            token_type_ids = torch.LongTensor(token_type_ids).to(device)
            added_fts = torch.FloatTensor(added_fts).to(device)
            cum_pred = torch.LongTensor(cum_pred).to(device)
            prev_pred = torch.LongTensor(prev_pred).to(device)
            y = torch.LongTensor(labels).to(device)
            # set the gradients to zero
            optimizer.zero_grad()
            # predict
            y_pred = model(X, mask_attention, token_type_ids, added_fts, cum_pred, prev_pred)
            # BP
        ...
        for rid, sample in enumerate(dataset.train_lst):
            if rid+1 != len(dataset.train_lst) and rid+1 not in dataset.idx_start_train:
                ...
                dataset.train_lst[rid+1]['prev_pred'] = list(list_probPred_train[rid])
...
np.save('pred_model_4_dev.npy', np.array(best_pred_dev))
np.save('pred_model_4_test.npy', np.array(best_pred_test))
