...
def model_eval(lst_data, model, dataiter):
    ...
    list_probPred = []
    model.eval()
    for sents, labels, mask_attention, token_type_ids, added_fts in data_iter(lst_data, batch_size=4, is_shuffle=False):
        ...
        # predict
        y_pred_ary = torch.exp(model(X, mask_attention, token_type_ids, added_fts)).detach().cpu().numpy()
...
if __name__ == "__main__":
    ...
    with open('tuning_model1.txt', 'a') as f:
...
np.save('pred_model_1_dev.npy', np.array(best_pred_dev))
np.save('pred_model_1_test.npy', np.array(best_pred_test))
