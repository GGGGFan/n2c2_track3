...
class N2C2_track3_dataset(Dataset):
    ...
    def __init__(self, args):
        if args.mode_train == True:
            ...
            if args.path_test_df is not '': # test data is available
                ...
                with open(args.path_train_metamap, 'rb') as handle:
                    self.ner_metamap_training = pickle.load(handle)
                with open(args.path_test_metamap, 'rb') as handle:
                    self.ner_metamap_test = pickle.load(handle)
            else:
                ...
                with open(args.path_train_metamap, 'rb') as handle:
                    self.ner_metamap_training = pickle.load(handle)
                with open(args.path_dev_metamap, 'rb') as handle:
                    self.ner_metamap_test = pickle.load(handle)
        else:
            # dev mode
            ...
            if args.path_test_df is not '': # test data is available
                ...
                with open(args.path_train_metamap, 'rb') as handle:
                    self.ner_metamap_training = pickle.load(handle)
                with open(args.path_dev_metamap, 'rb') as handle:
                    self.ner_metamap_dev = pickle.load(handle)
                with open(args.path_test_metamap, 'rb') as handle:
                    self.ner_metamap_test = pickle.load(handle)
            else:
                print("Test data is not provided. Will split train data as df_train and df_dev and use dev data to test!")
                self.df_train, self.df_dev, self.added_fts_tr, self.added_fts_dv, \
                self.ner_metamap_training, self.ner_metamap_dev = self.split_train_data(args)
                self.df_test = pd.read_csv(args.path_dev_df)
                self.added_fts_te = np.load(args.path_dev_add)
                with open(args.path_dev_metamap, 'rb') as handle:
                    self.ner_metamap_test = pickle.load(handle)
        ...
        with open(args.path_dic_cui_chapter, 'rb') as handle:
            self.dic_cui_chapter = pickle.load(handle)
    ...
    def split_train_data(self, args):
        ...
        with open(args.path_train_metamap, 'rb') as handle:
            ner_metamap_training = pickle.load(handle)
        ...
        return df_train.iloc[idx_tr].reset_index(drop=True), df_train.iloc[idx_dev].reset_index(drop=True), \
    added_fts_tr[idx_tr], added_fts_tr[idx_dev],\
    [ner_metamap_training[i] for i in idx_tr], [ner_metamap_training[i] for i in idx_dev]
    ...
    def convert_to_list(self):
        ...
        for i, df_data in enumerate(data_iter):
            ...
            for rid, row in df_data.iterrows():
                ...
                # concept encoding
                encoded_dict['concept_type'] = [0]*args.model_max_length
                if i == 0:
                    lst_concept = [c[1].split('"')[3] for c in self.ner_metamap_training[rid][0]+self.ner_metamap_training[rid][1]]
                    lst_cui = [c[0] for c in self.ner_metamap_training[rid][0]+self.ner_metamap_training[rid][1]]
                if i == 1:
                    lst_concept = [c[1].split('"')[3] for c in self.ner_metamap_test[rid][0]+self.ner_metamap_test[rid][1]]
                    lst_cui = [c[0] for c in self.ner_metamap_test[rid][0]+self.ner_metamap_test[rid][1]]
                if i == 2:
                    lst_concept = [c[1].split('"')[3] for c in self.ner_metamap_dev[rid][0]+self.ner_metamap_dev[rid][1]]
                    lst_cui = [c[0] for c in self.ner_metamap_dev[rid][0]+self.ner_metamap_dev[rid][1]]
                lst_zip = list(zip(lst_concept, lst_cui))
                for tp in lst_zip:
                    if tp[1] in self.dic_cui_chapter and tp[0].lower() != 'plan':
                        lst_c_tokens = self.tokenizer.encode(tp[0])[1:-1]
                        for lid, st in enumerate(encoded_dict['input_ids']):
                            l = 0
                            while l < len(lst_c_tokens):
                                if encoded_dict['input_ids'][lid+l] != lst_c_tokens[l]:
                                    break
                                l += 1
                                if l == len(lst_c_tokens):
                                    encoded_dict['concept_type'][lid:lid+l] = [self.dic_cui_chapter[tp[1]]]*l
            if i == 0:
...
class Transformer_base(nn.Module):
    ...
    def forward(self, x, mask_attention, token_type_ids, added_fts, concept_type):
        # hidden states of pretrained transformer
        out = self.tfm(input_ids=x,
                       attention_mask=mask_attention,
                       token_type_ids=token_type_ids,
                       pos_tag_ids=concept_type).last_hidden_state[:,0,:]
        # disposition
...
def data_iter(data, batch_size, is_shuffle=True):
    ...
    for i in range(batch_num):
        ...
        concept_type = [data[i * batch_size + b]['concept_type'] for b in range(cur_batch_size)]
        yield sents, labels, mask_attention, token_type_ids, added_fts, concept_type
...
def model_eval(lst_data, model, dataiter):
    ...
    list_probPred = []
    model.eval()
    for sents, labels, mask_attention, token_type_ids, added_fts, concept_type in data_iter(lst_data, batch_size=4, is_shuffle=False):
        X = torch.LongTensor(sents).to(device)
        mask_attention = torch.LongTensor(mask_attention).to(device)
        token_type_ids = torch.LongTensor(token_type_ids).to(device)
        concept_type = torch.LongTensor(concept_type).to(device)
        added_fts = torch.FloatTensor(added_fts).to(device)
        # predict
        y_pred_ary = torch.exp(model(X, mask_attention, token_type_ids, added_fts, concept_type)).detach().cpu().numpy()
...
if __name__ == "__main__":
    ...
    for ep in range(args.epoch):
        ...
        train_loss = 0
        num_batches = 0
        for sents, labels, mask_attention, token_type_ids, added_fts, concept_type in data_iter(dataset.train_lst, batch_size=args.batch_size):
            # convert to tensors
            X = torch.LongTensor(sents).to(device)
            mask_attention = torch.LongTensor(mask_attention).to(device)
            token_type_ids = torch.LongTensor(token_type_ids).to(device)
            ...
            optimizer.zero_grad()
            # predict
            y_pred = model(X, mask_attention, token_type_ids, added_fts, concept_type)
            # BP
        ...
        train_f1, _, _, _ = model_eval(dataset.train_lst, model, data_iter)
        print(f"Epoch {ep}\nTraining loss: {train_loss}\tTraining F1: {train_f1}")
        # performance on dev set
        dev_f1, _, _, res_prob_dev = model_eval(dataset.dev_lst, model, data_iter)
        print(f"\nDev F1: {dev_f1}")
        # performance on dev set
        test_f1, _, _, res_prob_test = model_eval(dataset.test_lst, model, data_iter)
        print(f"\nTest F1: {test_f1}\n\n")

        if dev_f1 > best_dev:
            best_pred_dev, best_pred_test = res_prob_dev, res_prob_test

        with open('tuning_model6.txt', 'a') as f:
            f.write(f"args: {vars(args)}")
            f.write(f"\nDev F1: {dev_f1}")
            f.write(f"\nTest F1: {test_f1}\n\n")

np.save('pred_model_6_dev.npy', np.array(best_pred_dev))
np.save('pred_model_6_test.npy', np.array(best_pred_test))
