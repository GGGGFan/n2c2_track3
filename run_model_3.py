...
class Transformer_base(nn.Module):
    ...
    def forward(self, x, mask_attention, token_type_ids, added_fts, prev_pred):
        ...
        # disposition
        out = self.linear_1(torch.cat([out, added_fts, prev_pred], dim=1))
        out = self.drop_1(out)
...
def data_iter(data, batch_size, is_shuffle=True):
    ...
    for i in range(batch_num):
        ...
        yield sents, labels, mask_attention, token_type_ids, added_fts, prev_pred
...
def model_eval(lst_data, model, dataiter, mode='train'):
    ...
    model.eval()
    if mode == 'train':
        for sents, labels, mask_attention, token_type_ids, added_fts, prev_pred in data_iter(lst_data, batch_size=4, is_shuffle=False):
            ...
            # predict
            y_pred_ary = torch.exp(model(X, mask_attention, token_type_ids, added_fts, prev_pred)).detach().cpu().numpy()
...
if __name__ == "__main__":
    ...
    print("Device:", device)
    with open('tuning_model3.txt', 'a') as f:
    ...
    for rid, sample in enumerate(dataset.train_lst):
        if rid+1 not in dataset.idx_start_train and rid+1 != len(dataset.train_lst):
    ...
    for ep in range(args.epoch):
        ...
        train_loss = 0
        num_batches = 0
        for sents, labels, mask_attention, token_type_ids, added_fts, prev_pred in data_iter(dataset.train_lst, batch_size=args.batch_size):
            # convert to tensors
            X = torch.LongTensor(sents).to(device)
            mask_attention = torch.LongTensor(mask_attention).to(device)
            token_type_ids = torch.LongTensor(token_type_ids).to(device)
            ...
            optimizer.zero_grad()
            # predict
            y_pred = model(X, mask_attention, token_type_ids, added_fts, prev_pred)
            # BP
        ...
        # add predictions to samples' following sections
        for rid, _ in enumerate(dataset.train_lst):
            if rid+1 not in dataset.idx_start_train and rid+1 != len(dataset.train_lst):
...
np.save('pred_model_3_dev.npy', np.array(best_pred_dev))
np.save('pred_model_3_test.npy', np.array(best_pred_test))
